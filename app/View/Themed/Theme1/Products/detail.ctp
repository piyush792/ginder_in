...
<?php
  $modifiedDate = $productDetail['Product']['modified'];
  $productUserId = $productDetail['Product']['user_id'];
if($productDetail['Product']['featured_product']==0){
  $expiryDate = date('d-M-Y', strtotime("+28 days", strtotime($modifiedDate)));
}else{
  $expiryDate = date('d-M-Y', strtotime("+2 months", strtotime($modifiedDate)));
}
// echo $expiryDate;

$fiveDaysBefore = strtotime($expiryDate. ' - 5 days');
$today = strtotime("today midnight");
// echo $fiveDaysBefore; exit;

$loggedInUserId=$this->Session->read("userId");
// echo "id: ".$loggedInUserId;

// if($today > $fiveDaysBefore){
if($loggedInUserId == $productUserId){
    ?>
    <div class="page-message info-message">Your ad is <b>expiring on <?=$expiryDate;?></b> and will be inactive after that. Please renew your ad by 
      <a href="./../../../users/post_an_edit/<?php echo $productDetail['Product']['id']; ?>">clicking here</a>
    </div>
    <?php
}
//}
...
<div class="page-breadcrumb"> <span class="page-breadcrumb-item"><?php echo $this->Html->link('Home', '/index');?></span> <span class="page-breadcrumb-item"><?php echo '<a href="'.SITE_URL.'products/show_category/'.$cat_name['Category']['id'].'">'.$cat_name['Category']['name'].'</a>'; ?></span> <span class="page-breadcrumb-currentitem">
  <?php
      $link = $sub_cat_name['Category']['id']."/".$cat_name['Category']['name'];
  ?>
  <?php echo '<a href="'.SITE_URL.'products/view/'.$link.'">'.$sub_cat_name['Category']['name'].'</a>'; ?> </span> </div>
<div class="middle-content-advert">
  ...
  <div class='product-page-left-sidebar'>
    ...
    <?php 
        $image_exist = PRODUCT_IMAGE_UPLOAD.$productDetail['Product']['image'];  // get the product image
        $gallery_image_exist = PRODUCT_IMAGE_UPLOAD.$productDetail['ProductImage'][0]['product_image'];  // get the first gallery image
        ...
        else if(file_exists($gallery_image_exist) && $productDetail['ProductImage'][0]['product_image']!=''){
          echo $this->Html->image(SITE_URL.PRODUCT_IMAGE_UPLOAD.$productDetail['ProductImage'][0]['product_image'],array('width'=>'381','height'=>'360'));
        }
        else{
    ...
    <div class='gallery product-thumbnail-image-wrapper'>
      ...
      <li class='thumbnail-img'>
        ...
        if(file_exists($image_exist) && $productDetail['Product']['image']!=''){
            ?>
            <a href="<?php echo SITE_URL.PRODUCT_IMAGE_UPLOAD.'/'.$productDetail['Product']['image'];?>"> <?php echo $this->Html->image(SITE_URL.PRODUCT_IMAGE_UPLOAD.'/'.$productDetail['Product']['image'],array('width' =>'119px','height'=>'119px','alt'=>''));?></a>
            <?php
        }
        else{
            ?>
            <a href="<?php echo SITE_URL.PRODUCT_IMAGE_UPLOAD.'no_image.jpg'?>"> <?php echo $this->Html->image(SITE_URL.PRODUCT_IMAGE_UPLOAD.'no_image.jpg',array('width' =>'119px','height'=>'119px','alt'=>'')); ?></a>
            <?php
        }
        ?>
        </li>
        ...
        {
        ?>
        <li class='thumbnail-img'> <a href="<?php echo SITE_URL.PRODUCT_IMAGE_UPLOAD.'large/'.$galleryimage['product_image'];?>"> <?php echo $this->Html->image(SITE_URL.PRODUCT_IMAGE_UPLOAD.'thumbnail/'.$galleryimage['product_image'],array('width' =>'119px','height'=>'119px','alt'=>'')); ?> </a> </li>
        <?php
        }
        ?>
      </ul>
  ...
  <section class='product-page-right-sidebar'>
    <header>
      <h1 class='product-title'><?php echo strtoupper($productDetail['Product']['name']);?></h1>
    </header>
<section class='prdouct-description-wrapper product-description-dateattr-wrapper'>
      <div class="prdouct-description-heading product-description-dateattr">Published Date:
      <span class='product-description-content product-description-dateval'>
        <?php
                 $currentDate = date('m/d/Y h:i:s a', time());
                 $date1 = $currentDate;
                 $date2 = $productDetail['Product']['created'];
        ...
        <?php //print time2string(time()-strtotime('2010-04-28 17:25:43')).' ago'; ?>
      		</span>
	</div>

	
	 <!-- <div id="loader_report1">&nbsp;Loading...</div> -->
            <div id="loader_report1" class="prdouct-description-heading product-description-dateattr">Views
                <!-- <span id="loader_report1"></span> -->
                <span id="counter" class="product-description-content product-description-viewval"></span>
            </div>
    ...
    <div class='product-owner-details'>
      ...
      <div class='right-sidebar'>
        ...
        <form method="post" action="">
          <div class='captcha-wrapper'> <?php echo $this->Form->input('captcha', array('id'=>'captcha', 'label'=> false, 'div'=> false, 'class' => "captcha-field"));?>
    ...
    <section class='product-attribute-wrapper'>
      ...
      <ul class='product-attribute-content'>
        ...
        <li class='attribute-item'>
          <div class='attribute-label'> <strong><?php echo $value['name']?>:</strong> </div>
          ...
          <?php
                            ...
                            ?>
          <div class='attribute-value'> <?php echo ucfirst($val['name']);?> </div>
          <?php
                            }
                            ?>
        </li>
        ...
        <li class='attribute-item'>
          ...
          <div class='attribute-value'><span class="product-price">Rs.
            <?=$productDetail['Product']['price'];?>
            </span> <Span class="price-tag">Fixed price</span></div>
        </li>
      </ul>
    </section>
    ...
    <!-- product share start -->
    <div class="product-share-wrapper"> <span class="product-share-heading">Share at:</span> <span> <a title="Facebook" onclick="window.open('http://www.facebook.com/sharer.php?u=http://ginder.in/products/detail/<?=$productDetail['Product']['id']; ?>&amp;t='+encodeURIComponent(document.title)+'', 'facebook', 'width=650,height=400, left=100,top=100,scrollbars=yes');" href="JavaScript:void(0);" class="socialbookmarks_link"><?php echo $this->Html->image(SITE_URL.'img/icon-facebook.png');?></a> </span> <span> <a href="https://twitter.com/share" class="twitter-share-button" data-url="http://ginder.in/products/detail/<?= $productDetail['Product']['id'] ?>" data-text="<?= $productDetail['Product']['name'] ?>" data-count="none"><?php echo $this->Html->image(SITE_URL.'img/icon-twitter.png');?></a> </span> <span> 
        ...
        <div class="modal-content">
          <div class="modal-body">
            ...
            <div class="modal-form-content">
              ...
              <div class="modal-overview"> We always ensure to serves our users better. But still if any content is inappropriate,  spam, 
                against community and person please share your thoughts with us. </div>
              <div class="modal-form">
                <div class="modal-form-row"> <?php echo $this->Form->input('id',array('id'=>'product_id','type'=>'hidden','value'=>$productDetail['Product']['id'], 'class'=>"ctrl-singleline", 'div'=>false));?>
                  <textarea name="description" id="description" class="ctrl-multiline"></textarea>
                </div>
                <div class="modal-form-row">
                  <div class="col-sm-12 col-lg-5 modal-form-col float-left"> <?php echo $this->Form->input('user_name',array('id'=>'user_name','class'=>"required form-control", 'div'=>false, 'placeholder'=>'Name *', 'label'=>false, 'required'));?> 
                  ...
                  <div class="col-sm-12 col-lg-5 modal-form-col modal-rightcol-field float-left"> <?php echo $this->Form->input('user_email',array('id'=>'user_email','class'=>"validate-email required form-control", 'div'=>false, 'placeholder'=>'Email *', 'label'=>false, 'required'));?> 
            ...
            <div class="confirmation-msg-wrapper">
              ...
              <div class="confirmation-button">
                <button class="btn-teritory">Ok</button>
              </div>
            </div>
        ...
        <div class="modal-content">
          <div class="modal-body">
            ...
            <div class="modal-form-content">
              ...
              <div class="modal-overview"> <b><?php echo $productDetail['Product']['name'];?></b> </div>
              <div class="modal-form">
                <div class="modal-form-row"> <?php echo $this->Form->input('id',array('id'=>'user_product_id','type'=>'hidden','value'=>$productDetail['Product']['id'], 'class'=>"ctrl-singleline", 'div'=>false));?>
                  <textarea name="user_enquiry_description" id="user_enquiry_description" class="ctrl-multiline"></textarea>
                </div>
                <div class="modal-form-row">
                  <div class="col-sm-12 col-lg-5 modal-form-col float-left"> <?php echo $this->Form->input('user_enquiry_name',array('id'=>'user_enquiry_name','class'=>"ctrl-singleline", 'div'=>false, 'placeholder'=>'Name *', 'label'=>false, 'required'));?> 
                    <!-- <div class="warning-wrapper-field error-msg-field">
                                            <span class="glyphicon glyphicon-alert"></span>Cannot leave name field blank.
                                        </div> --> 
                  </div>
                  <div class="col-sm-12 col-lg-5 modal-form-col modal-rightcol-field float-left"> <?php echo $this->Form->input('user_enquiry_email',array('id'=>'user_enquiry_email','class'=>"validate-email required ctrl-singleline", 'div'=>false, 'placeholder'=>'Email *', 'label'=>false, 'required'));?> 
                    <!-- <div class="warning-wrapper-field error-msg-field">
                                            <span class="glyphicon glyphicon-alert"></span> Cannot leave email field blank.
                                        </div> --> 
                  </div>
                ...
                <div class="modal-form-row">
                  <div class="col-sm-12 col-lg-5 modal-form-col float-left"> <?php echo $this->Form->input('user_enquiry_mobile',array('id'=>'user_enquiry_mobile','class'=>"ctrl-singleline", 'div'=>false, 'placeholder'=>'Mobile', 'label'=>false));?> </div>
                  <div class="col-sm-12 col-lg-5 modal-form-col modal-rightcol-field float-left"> <?php echo $this->Form->input('user_enquiry_offer',array('id'=>'user_enquiry_offer','class'=>"ctrl-singleline", 'div'=>false, 'placeholder'=>'Offer', 'label'=>false));?> </div>
                </div>
                <div class="modal-form-footer"> <?php echo $this->Form->button("Send Message",array('id'=>'interested_submit', 'type'=>'button',"label"=>false,"div"=>false,'class'=>'btn-primary btn-mid-size'));?>
                  <button class="btn-teritory btn-mid-size btn-modal-teritory">Close</button>
                </div>
              </div>
            </div>
...
<section class="product-premium-ads-wrapper">
  <header>
    <h2 class="section-heading">Similar ads related to this category</h2>
  </header>
  <ul class="premiumbxslider">
    ...
    <li>
      <div class="card">
        ...
        <div class="card-body"> <a class="card-title" href="<?php echo SITE_URL."products/detail/".$similar_products[$i]['Product']['id'];?>"> <?php echo $similar_products[$i]['Product']['name'];?>
          <h4>
            <?php

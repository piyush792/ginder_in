 ...
 */
    protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
        ...
        if ($this->{$check}($model)) {
            ...
            unset($options['rel']);
            $activelink = $this->Html->tag($tag, $title, array_merge($options, compact('escape', 'class')));
            
            if($title=="Next &rarr;")
            {
                $activelink = str_replace('<li class="next disabled">',"",$activelink);
                $activelink = str_replace('</li>',"",$activelink);
                $activelink = '<li class="next disabled"><a href="#">'.$activelink."</a></li>";
            }
            else
            {
                $activelink = str_replace('<li class="prev disabled">',"",$activelink);
                $activelink = str_replace('</li>',"",$activelink);
                $activelink = '<li class="prev disabled"><a href="#">'.$activelink."</a></li>";
            }
            return $activelink;
        ...
        if ($modulus && $params['pageCount'] > $modulus) {
            ...
            for ($i = 1; $i <= $params['pageCount']; $i++) {
                if ($i == $params['page']) {
                    ...
                    $activelink = $this->Html->tag($tag, $i, array('class' => $currentClass));
                    $activelink = str_replace('<li class="active">',"",$activelink);
                    $activelink = str_replace('</li>',"",$activelink);
                    $activelink = '<li class="active"><a href="#">'.$i."</a></li>";

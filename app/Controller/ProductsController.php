...
Class ProductsController extends AppController {
    ...
    var $uses = array("Product", "ProductAttribute", "ProductAttributeRelation", "ProductAttributeValue", "Category", "CategoriesProduct", "WishList", "User",
        "ProductImage", "EmailTemplate", "Rating", "ProductContact","State","City", "SocialLink", "SpamReport", "VisiterCount", "OrderTransactions");
    ...
    public function admin_transactionsearchurl() //For transaction search
    {
        $searchStr = "";
        if(isset($this->params['named']['sort']) && $this->params['named']['sort'] != "" && isset($this->params['named']['direction']) && $this->params['named']['direction'] != "")
        {
        ...
        $this->redirect('/admin/products/transaction_lists'.$searchStr);
    }

    public function admin_manageproductsearchurl()
    {
        $searchStr = "";
        if(isset($this->params['named']['sort']) && $this->params['named']['sort'] != "" && isset($this->params['named']['direction']) && $this->params['named']['direction'] != "")
        {
        ...
        {
            list($month,$day,$year) =  split('/',$this->params['url']['start_date']);
            $searchStr.= '/start_date:'.date($year.'-'.$month.'-'.$day);
        }  

        if(isset($this->params['url']['end_date']) && $this->params['url']['end_date'] !="")
        {
            list($month,$day,$year) =  split('/',$this->params['url']['end_date']);
            $searchStr.= '/end_date:'.date($year.'-'.$month.'-'.$day);
        }  

        $this->redirect('/admin/products/manage_products'.$searchStr);
    }
    ...
    public function admin_manage_products(){
        ...
        if(isset($this->params['named']['sort']) && $this->params['named']['sort'] != "" && isset($this->params['named']['direction']) && $this->params['named']['direction'] != "")
        {
            $action = 'manageproductsearchurl/sort:'.$this->params['named']['sort'].'/direction:'.$this->params['named']['direction'];
        }else{
            $action =  'manageproductsearchurl';
        }
        $this->set(compact("action"));
        ...
        if(isset($this->params['named']['start_date']) && isset($this->params['named']['end_date'])){
            $searchStartKey = $this->params['named']['start_date'];
            $searchEndKey = $this->params['named']['end_date'];        
            $condition_search = array('Product.created BETWEEN ? AND ?' => array($searchStartKey,$searchEndKey));
        }
        ...
        // $condition_search_paid = array('AND' => array('Product.featured_product' => 1));

        // $fieldArr = array('Product.id', 'Product.name', 'Product.price', 'Product.image', 'Product.active', 'Product.created', 'Category.name as cat_name', 'ProductContact.name as contact_name', 'ProductImages.product_image');
        $fieldArr = array('Product.id', 'Product.name', 'Product.price', 'Product.image', 'Product.active', 'Product.created');
        $conditions = array("conditions" => array($condition_search, $condition_search_status),
            'limit' => PAGING,
            'fields' => $fieldArr,
            'order' => 'Product.id DESC',
            //'group' => 'Product.id'
        ...
        $totalProducts = $this->Product->find('count', array("conditions" => array($condition_search, $condition_search_status)));
        $this->set("totalProducts",$totalProducts);        
    }

    public function admin_transaction_lists(){
        $condition_search = '';
        ...
        $fieldArr = array('User.email', 'Product.name', 'OrderTransactions.card_num', 'OrderTransactions.card_cvc', 'OrderTransactions.card_exp_month', 'OrderTransactions.card_exp_year', 'OrderTransactions.transaction_amount', 'OrderTransactions.transaction_amount_currency', 'OrderTransactions.transaction_id', 'OrderTransactions.payment_status', 'OrderTransactions.created');
        $conditions = array("conditions" => array($condition_search, $condition_search_location, $condition_search_status),
            'limit' => PAGING,
            'fields' => $fieldArr,
            'order' => 'OrderTransactions.id DESC',
            'group' => 'OrderTransactions.id',
            "joins" => array(
                array('table' => 'products',
                    'alias' => 'Product',
                    'type' => 'LEFT',
                    'conditions' => array('OrderTransactions.product_id = Product.id')),
                array('table' => 'users',
                    'alias' => 'User',
                    'type' => 'INNER',
                    'conditions' => array('OrderTransactions.user_id = User.id')
                )
            ));

        $this->paginate = $conditions;
        $orderTransactionsList = $this->paginate('OrderTransactions');
        $this->set("orderTransactionsList",$orderTransactionsList);
        // echo "<pre>";
        // print_r($orderTransactionsList);
        // exit;
    }

    public function admin_ajaxDeleteProduct() {
        $this->autoRender = false;

        if(isset($this->request->data['checkbox'][0])){
            foreach($this->request->data['checkbox'] as $productId){
                
                // Start to delete Gallery Images
                $galleryimages = $this->ProductImage->find('all', array('conditions' => array('ProductImage.product_id' => $productId)));
                // echo "<pre>";
                // print_r($galleryimages);
                foreach($galleryimages as $galleryimage){
                    $image_path = WWW_ROOT . PRODUCT_IMAGE_UPLOAD . $galleryimage['ProductImage']['product_image'];
                    $image_path2 = WWW_ROOT . PRODUCT_IMAGE_UPLOAD . '/thumbnail/' . $galleryimage['ProductImage']['product_image'];
                    $image_path3 = WWW_ROOT.PRODUCT_IMAGE_UPLOAD.'/large/'.$galleryimage['ProductImage']['product_image'];
                    if(file_exists($image_path) && file_exists($image_path2) && file_exists($image_path3)){
                        @unlink($image_path);
                        @unlink($image_path2);
                        @unlink($image_path3); 
                    } elseif (file_exists($image_path) && file_exists($image_path2)) {
                        @unlink($image_path);
                        @unlink($image_path2);
                    } elseif (file_exists($image_path)) {
                        @unlink($image_path);
                    } else {
                        $data = "<div class='alert alert-danger'>Product Gallery Image Not Found.</div>";
                    }

                    $this->ProductImage->delete($galleryimage['ProductImage']['id']);
                }
                // End to delete Gallery Images

                // Start to delete Product Image
                $productImage = $this->Product->find('first', array('conditions' => array('Product.id' => $productId)));                
                // echo "<pre>";
                // print_r($productImage);exit;

                $image_path = WWW_ROOT . PRODUCT_IMAGE_UPLOAD . $productImage['Product']['image'];
                $image_path2 = WWW_ROOT . PRODUCT_IMAGE_UPLOAD . '/thumbnail/' . $productImage['Product']['image'];
                $image_path3 = WWW_ROOT.PRODUCT_IMAGE_UPLOAD.'/large/'.$productImage['Product']['image'];
                if(file_exists($image_path) && file_exists($image_path2) && file_exists($image_path3)){
                    @unlink($image_path);
                    @unlink($image_path2);
                    @unlink($image_path3);
                ...
                if ($this->Product->delete($productId)) {
                    $data = "<div class='alert alert-danger'>Product Image Deleted Successfully.</div>";
                } else {
                    $data = "Product Image Can't Deleted, Please Try Again!";
                }
                // End to delete product Image
            }

            $totalProducts = $this->Product->find('all', array('order' => 'Product.id'));   
            echo count($totalProducts);
        }
    }
    ...
    public function product_list($parent_category_id = null, $current_city = null) {
        $this->autoLayout = false;

        // echo "name: ".$current_city; exit;
        ...
        if (!empty($ArraySessionChecked['order_by'])) {
            if ($ArraySessionChecked['order_by'] == 'newest') {
                $orderby = 'City.name ="'.$current_city.'" desc, Product.created DESC';
            } elseif ($ArraySessionChecked['order_by'] == 'highest') {
                $orderby = 'City.name ="'.$current_city.'" desc, Product.price DESC';
            } elseif ($ArraySessionChecked['order_by'] == 'lowest') {
                $orderby = 'City.name ="'.$current_city.'" desc, Product.price ASC';
            } elseif ($ArraySessionChecked['order_by'] == 'rating') {
                $orderby = 'City.name ="'.$current_city.'" desc, Product.rating DESC';
            }
        } else {
            $orderby = 'City.name ="'.$current_city.'" desc, Product.created DESC';
        }
        ...
        //Featured Products
        $not_featured_condition='';
        $not_featured_condition = array("Product.featured_product" => 0);

        //Recent Products
        $this->paginate = array('Product' => array('all',
                "fields" => array('DISTINCT Product.id', 'Product.name', 'Product.image', 'Product.description', 'Product.price', 'Product.new_product', 'City.name', 'ProductImages.product_image' /* ,'Product.rating_avg' */),
                'limit' => 9,
                'order' => $orderby,
                'joins' => array(
                    array('table' => 'categories_products',
                ...
                'conditions' => array($product_condition, $not_featured_condition),
                'group' => 'Product.id'
        ));
    ...
    public function admin_premium_ads_expire() {
        // echo "<pre>";
        ...
        if(isset($this->request->data['product_id'])){
            foreach($this->request->data['product_id'] as $key=>$value)
            {
                ...
                // exit;
                $this->request->data['Product'][$value]['featured_product'] = 0;
    ...
    public function admin_premium_ads_reminder_mail() {
    ...
    public function admin_unpaid_ads_expire() {
    ...
    public function admin_unpaid_ads_reminder_mail() {
